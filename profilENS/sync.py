...
def add_or_update(redis_db, iterator):
    ...
    redis_db.set("clipper:status", "db")

    for user in User.objects.all():
        current[user.username] = user

    redis_db.set("clipper:status", "sync")
    message = "[ SYNCING ]"
    ...
    for line in iterator:
        ...
        if username in current.keys():
            ...
            user.save()
        redis_db.lpush("clipper:update", username)
...
def import_from_clipper(redis_db):
    ...
    redis_db.set("clipper:status", "ssh")
    ...
    if err is not None:
        ...
        redis_db.set("clipper:n_total_user", len(splitted))
...
def sync_with_clipper():
    ...
    try:
        ...
        if redis_db.get("clipper:lock")==b'True':
            TODO_LOGGING("Database lock still in place")
            raise Exception()
        else:
            redis_db.set("clipper:lock", True)
            redis_db.delete("clipper:status")
        ...
        redis_db.set("clipper:lock", False)
        redis_db.delete("clipper:status")
        redis_db.delete("clipper:update")
        redis_db.delete("clipper:n_user")
        redis_db.delete("clipper:n_total_user")
        TODO_LOGGING("[DONE]")
